...
shp <- readOGR("Daten/kreise/kreise.shp") # Shapefile laden 
plot(shp) # Übersichtsplot
head(shp@data) # Datensatz ansehen

shp <- spTransform(shp, CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")) # Projektion der Geodaten anpassen
shp <- shp[shp@data$name2 %in% "Nordrhein-Westfalen",]# nur Kreise aus NRW aussieben
plot(shp) # Übersichtsplot
writeOGR(shp, "kreise_nrw.geojson", layer="kreise_nrw", driver="GeoJSON") # neue Geodaten als Geojson speichern
